...
### 2.1 Reference data and PSF

The `pandeia` python package, you will need to download
the respective (1) [reference
data](https://stsci.box.com/v/pandeia-data-v2026p2-jwst) and (2) [PSF](https://stsci.box.com/v/pandeia-psfs-v2026p2-jwst) files.

After unzipping them, make sure the environmental variables
`$pandeia_refdata` and `$PSF_DIR` point to the respective folders
containing `VERSION_DATA` and `VERSION_PSF` in the respective files.

If this is your first time using pandeia, you likely need to setup the
`$pandeia_refdata` and `$PSF_DIR` in your `~/.bashrc`,
...
/home/USERNAME/data/.../pandeia_data-2025.7-jwst

echo $PSF_DIR
/home/USERNAME/data/.../pandeia_psfs-2026.2-jwst
...
wget https://archive.stsci.edu/hlsps/reference-atlases/hlsp_reference-atlases_hst_multi_other-spectra_multi_v2_sed.tar
```

For the BT-Settl spectra you will need to move the files, something like this:

```bash
mv grp/redcat/trds/source/ grp/redcat/trds/grid/phoenixBTS11_15/
```
