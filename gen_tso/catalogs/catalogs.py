...
class Catalog():
    ...
    def __init__(self, custom_targets=None):
        ...
        # Check for custom targets (TBD: revise logic)
        ...
        custom_catalog = os.path.join(ROOT, 'data', 'custom_targets_session.txt')
        if load_session is None or not os.path.exists(custom_catalog):
            custom_catalog = None
        ...
        if custom_catalog is None:
            ...
            if os.path.exists(custom_path):
                custom_catalog = custom_path

        if custom_catalog is not None:
            custom = load_targets(custom_catalog, is_confirmed=True)
